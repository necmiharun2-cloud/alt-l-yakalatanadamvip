    ...
    <div className="space-y-6">
      {profile ? (
        <form onSubmit={handleSubmit} className="relative">
          <textarea
            ...
            className="w-full bg-[#151b27] border border-white/10 rounded-2xl p-5 pr-14 text-sm font-bold focus:outline-none focus:border-[#00e5ff] transition-colors resize-none h-24"
          />
          <button
            type="submit"
            disabled={loading || !newComment.trim()}
            className="absolute right-4 bottom-4 p-2 bg-[#00e5ff] text-black rounded-xl hover:bg-white transition-all disabled:opacity-50"
          >
            <Send size={18} />
          </button>
        </form>
      ...
      <div className="space-y-4">
        {comments.map((comment) => (
          <div key={comment.id} className="bg-[#151b27]/50 border border-white/5 p-5 rounded-2xl group transition-all hover:bg-[#151b27]">
            <div className="flex justify-between items-start mb-3">
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#00e5ff] to-[#e67e22] flex items-center justify-center">
                  <User size={14} className="text-black" />
                </div>
                <div>
                  <h4 className="text-sm font-black text-[#00e5ff]">{comment.userName}</h4>
