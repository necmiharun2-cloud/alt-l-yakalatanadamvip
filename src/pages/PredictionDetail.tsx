    ...
    return (
      <div className="min-h-screen bg-[#050505] text-white">
        ...
        <main className="max-w-7xl mx-auto py-40 px-4 text-center">
          ...
          <Link to="/tahminler" className="text-[#ff0000] mt-4 block underline">Tüm tahminlere dön</Link>
        </main>
        <Footer />
      </div>
    );
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="w-full lg:w-1/4 space-y-8">
            <div className="flex flex-col items-center p-6 bg-[#0a0a0a] border border-white/5 rounded-3xl">
              <div className="w-24 h-24 rounded-full overflow-hidden mb-4 border-2 border-[#ff0000]">
                <img 
              ...
              <div className="divide-y divide-white/5">
                {sidebarTahminler.map((t, i) => (
                  <Link 
                    key={i} 
                    to={`/tahmin/${t.slug}`}
                    className="block px-6 py-4 text-xs font-bold text-gray-400 hover:text-[#ff0000] hover:bg-white/5 transition-all"
                  >
                    {t.title}
                  </Link>
                ))}
          ...
          <div className="flex-1 bg-[#0a0a0a]/50 border border-white/5 p-6 md:p-10 rounded-3xl shadow-2xl">
             ...
             {prediction.subTitle && (
                <div className="bg-[#ff0000]/10 border-l-4 border-[#ff0000] text-sm p-4 mb-8 font-bold italic">
                   {prediction.subTitle}
                </div>
             )}
             ...
             <div className="relative">
                <div className={`text-sm md:text-base text-gray-300 leading-relaxed ${!showContent ? 'blur font-mono opacity-30 select-none' : ''}`}>
                    ...
                    {showContent && prediction.ayaklar && prediction.ayaklar.length > 0 && (
                      <div className="mb-8 w-full overflow-x-auto rounded-xl border border-[#ff0000] relative bg-[#000000] custom-scrollbar">
                        ...
                        <div className="relative z-10">
                          <div className="bg-[#111111] flex flex-col justify-center items-center py-3 border-b border-[#ff0000]/30">
                            <span className="text-[#ff0000] font-black tracking-widest text-lg md:text-xl uppercase">ALTILIYAKALATANADAM</span>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-6 divide-x divide-y md:divide-y-0 divide-[#ff0000]/30 text-center min-w-full md:min-w-[500px]">
                            {prediction.ayaklar.slice(0, 6).map((a: string, i: number) => {
                              const horseLines = typeof a === 'string' ? a.split('\n').map(line => line.trim()).filter(Boolean) : [];
                              return (
                                <div key={i} className="flex flex-col">
                                  <div className="bg-[#ff0000] text-black text-center py-1 text-xs md:text-sm font-black border-b border-[#111111]">
                                    {i + 1}. AYAK
                                  </div>
                                  <div className="bg-[#111111] text-white text-center py-0.5 text-[10px] md:text-xs font-bold shadow-inner border-b border-white/5">
                                    AT NO
                                  </div>
                                  <div className="flex flex-col p-2 space-y-2 flex-1 bg-transparent">
                                    {horseLines.map((line, idx) => (
                                      <div key={idx} className="flex items-center space-x-2 text-xs md:text-sm relative z-20">
                                        ...
                                        <span className={`font-bold tabular-nums truncate ${idx === 0 ? 'text-[#ff0000]' : 'text-gray-300'}`}>{line}</span>
                                      </div>
                                    ))}
                                    {horseLines.length === 0 && (
                    ...
                    {showContent && prediction.fiyat && (
                      <div className="mb-8 p-4 bg-[#ff0000]/20 border border-[#ff0000]/50 rounded-xl inline-block">
                         <span className="text-xs font-black uppercase text-[#ff0000]">Şablon Tutarı: </span>
                ...
                {!showContent && (
                  <div className="absolute inset-0 flex items-center justify-center p-4">
                    <div className="bg-black/90 p-8 rounded-2xl border border-[#ff0000]/20 text-center">
                        <Lock size={40} className="text-[#ff0000] mx-auto mb-4" />
                        <h4 className="font-bold mb-2">İçerik Kilitli</h4>
                        <a href="https://wa.me/905336711463" target="_blank" rel="noreferrer" className="text-[#ff0000] font-black underline text-sm uppercase">VIP Üye Ol</a>
                    </div>
                  </div>
                )}
             </div>
             ...
             {prediction.commentsEnabled !== false && (
                <div className="mt-12 pt-8 border-t border-white/5">
                   <h3 className="text-xl font-black italic uppercase mb-8 flex items-center space-x-3">
                      <MessageCircle size={24} className="text-[#ff0000]" />
