    ...
    return (
      <div className="min-h-screen bg-[#080d16] text-white">
        ...
        <main className="max-w-7xl mx-auto py-40 px-4 text-center">
          ...
          <Link to="/tahminler" className="text-[#00e5ff] mt-4 block underline">Tüm tahminlere dön</Link>
        </main>
        <Footer />
      </div>
    );
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="w-full lg:w-1/4 space-y-8">
            <div className="flex flex-col items-center p-6 bg-[#0a0a0a] border border-white/5 rounded-3xl">
              <div className="w-24 h-24 rounded-full overflow-hidden mb-4 border-2 border-[#00e5ff]">
                <img 
              ...
              <div className="divide-y divide-white/5">
                {sidebarTahminler.map((t, i) => (
                  <Link 
                    key={i} 
                    to={`/tahmin/${t.slug}`}
                    className="block px-6 py-4 text-xs font-bold text-gray-400 hover:text-[#00e5ff] hover:bg-white/5 transition-all"
                  >
                    {t.title}
                  </Link>
                ))}
          ...
          <div className="flex-1 bg-[#0a0a0a]/50 border border-white/5 p-6 md:p-10 rounded-3xl shadow-2xl">
             ...
             {prediction.subTitle && (
                <div className="bg-[#00e5ff]/10 border-l-4 border-[#00e5ff] text-sm p-4 mb-8 font-bold italic">
                   {prediction.subTitle}
                </div>
             )}
             ...
             <div className="relative">
                <div className={`text-sm md:text-base text-gray-300 leading-relaxed ${!showContent ? 'blur font-mono opacity-30 select-none' : ''}`}>
                    ...
                    {showContent && prediction.ayaklar && prediction.ayaklar.length > 0 && (
                      <div className="mb-8 w-full overflow-x-auto rounded-xl border border-[#00e5ff] relative bg-[#050A14] custom-scrollbar">
                        ...
                        <div className="relative z-10">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-6 divide-x divide-y md:divide-y-0 divide-[#00e5ff]/30 text-center min-w-full md:min-w-[500px]">
                    ...
                    {showContent && prediction.fiyat && (
                      <div className="mb-8 p-4 bg-[#00e5ff]/20 border border-[#00e5ff]/50 rounded-xl inline-block">
                         <span className="text-xs font-black uppercase text-[#00e5ff]">Şablon Tutarı: </span>
                ...
                {!showContent && (
                  <div className="absolute inset-0 flex items-center justify-center p-4">
                    <div className="bg-black/90 p-8 rounded-2xl border border-[#00e5ff]/20 text-center">
                        <Lock size={40} className="text-[#00e5ff] mx-auto mb-4" />
                        <h4 className="font-bold mb-2">İçerik Kilitli</h4>
                        <a href="https://wa.me/905336711463" target="_blank" rel="noreferrer" className="text-[#00e5ff] font-black underline text-sm uppercase">VIP Üye Ol</a>
                    </div>
                  </div>
                )}
             </div>
             ...
             {prediction.commentsEnabled !== false && (
                <div className="mt-12 pt-8 border-t border-white/5">
                   <h3 className="text-xl font-black italic uppercase mb-8 flex items-center space-x-3">
                      <MessageCircle size={24} className="text-[#00e5ff]" />
