    ...
    <div className="min-h-screen bg-[#080d16] text-white">
      ...
      <main className="max-w-7xl mx-auto py-20 px-4">
        ...
        <motion.div 
          ...
          className="bg-[#151b27] border-l-4 border-[#00e5ff] rounded-2xl p-8 mb-12 shadow-2xl relative overflow-hidden group"
        >
          <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
            <CreditCard size={120} />
          </div>
          <div className="flex items-center space-x-4 mb-6">
            <div className="bg-[#00e5ff] p-2 rounded-lg">
          ...
          <div className="flex flex-col md:flex-row gap-6 flex-wrap">
            {banks.map(bank => (
              <div key={bank.id} className="flex-1 min-w-[300px] bg-black/40 p-6 rounded-2xl border border-white/5">
                <p className="text-[#00e5ff] font-black text-lg mb-2">{bank.bankName}</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  ...
                  <select 
                    ...
                    className="w-full bg-[#151b27] border border-white/10 rounded-2xl p-4 focus:outline-none focus:border-[#00e5ff] transition-colors appearance-none"
                ...
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-[#151b27] border border-white/10 rounded-2xl p-4 focus:outline-none focus:border-[#00e5ff] transition-colors"
                ...
                <div>
                  <label className="block text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-2 flex items-center space-x-2">
                    <MessageCircle size={10} className="text-[#00e5ff]" />
                    <span>Dekont Yüklemesi (Zorunlu)</span>
                  </label>
                  <div className={`relative border-2 border-dashed rounded-2xl p-8 transition-all flex flex-col items-center justify-center text-center cursor-pointer group ${file ? 'border-[#00e5ff] bg-[#00e5ff]/5' : 'border-white/10 hover:border-white/20'}`}>
                    <input 
                      type="file" 
                      accept="image/*,.pdf"
                      required
                    ...
                    {!file ? (
                      ...
                      <div className="flex flex-col items-center">
                        <Check size={32} className="text-[#00e5ff] mb-4" />
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    value={formData.note}
                    className="w-full bg-[#151b27] border border-white/10 rounded-2xl p-4 focus:outline-none focus:border-[#00e5ff] transition-colors h-32 resize-none"
              ...
              {myPayments.length === 0 ? (
                 ...
                 <div className="space-y-4">
                   {myPayments.map((p, idx) => (
                      <div key={idx} className="bg-[#151b27] border border-white/5 rounded-2xl p-6 relative group overflow-hidden">
                         <div className="flex justify-between items-start mb-2">
                            <span className="text-[#00e5ff] font-black uppercase tracking-widest text-sm">{p.bank}</span>
                         ...
                         <div className="flex flex-col space-y-1">
                            ...
                            <div className="flex justify-between text-xs pt-1">
                               ...
                               <span className="text-[#00e5ff] font-black">₺ {p.amount}</span>
                            </div>
                         </div>
                      </div>
                   ))}
