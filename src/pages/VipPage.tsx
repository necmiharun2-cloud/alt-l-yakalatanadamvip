    ...
    <div className="min-h-screen bg-[#050505] text-white">
      ...
      <main className="max-w-7xl mx-auto py-24 px-4 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-16">
           <Star size={64} className="text-[#ff0000] mx-auto mb-6" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
           {packages.map((pkg, idx) => (
             <motion.div 
               ...
               className="bg-[#0a0a0a] border border-white/5 rounded-[40px] p-10 hover:border-[#ff0000]/30 transition-all shadow-2xl relative"
             >
                <h3 className="text-2xl font-black italic text-[#ff0000] mb-2 uppercase">{pkg.title}</h3>
                ...
                <Link to="/odeme-bildirimi" className="block w-full p-4 bg-[#1a1a1a] border border-white/10 rounded-2xl font-black uppercase text-xs hover:bg-[#ff0000] hover:text-black transition-all">Şimdi Satın Al</Link>
             </motion.div>
           ))}
        </div>

        <div className="mt-24 max-w-3xl mx-auto bg-[#1a1a1a] rounded-3xl p-8 flex items-start space-x-6">
           <Shield size={48} className="text-[#ff0000]" />
             ...
             <div className="space-y-4">
                {banks.map((bank, i) => (
                   <div key={i} className="bg-black/20 p-4 rounded-xl border border-white/5">
                      <p className="text-[#ff0000] font-black">{bank.bankName}</p>
