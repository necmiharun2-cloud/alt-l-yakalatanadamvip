    ...
    <div className="min-h-screen bg-[#080d16] text-white">
      ...
      <main className="max-w-7xl mx-auto py-24 px-4 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-16">
           <Star size={64} className="text-[#00e5ff] mx-auto mb-6" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
           {packages.map((pkg, idx) => (
             <motion.div 
               ...
               className="bg-[#0a0a0a] border border-white/5 rounded-[40px] p-10 hover:border-[#00e5ff]/30 transition-all shadow-2xl relative"
             >
                <h3 className="text-2xl font-black italic text-[#00e5ff] mb-2 uppercase">{pkg.title}</h3>
                ...
                <Link to="/odeme-bildirimi" className="block w-full p-4 bg-[#151b27] border border-white/10 rounded-2xl font-black uppercase text-xs hover:bg-[#00e5ff] hover:text-black transition-all">Şimdi Satın Al</Link>
             </motion.div>
           ))}
        </div>

        <div className="mt-24 max-w-3xl mx-auto bg-[#151b27] rounded-3xl p-8 flex items-start space-x-6">
             ...
             <div className="space-y-4">
                {banks.map((bank, i) => (
                   <div key={i} className="bg-black/20 p-4 rounded-xl border border-white/5">
                      <p className="text-[#00e5ff] font-black">{bank.bankName}</p>
